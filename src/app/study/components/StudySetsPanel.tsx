            ...
            <div className="flex items-center gap-2 w-full min-w-0">
              ...
              >
                <div className="flex items-center gap-2">
                  <span className="text-white truncate">{set.name}</span>
                  <span className="text-sm text-white whitespace-nowrap flex-shrink-0">{set.cards.length} cards</span>
                </div>
              </button>
              <button
                onClick={() => onEditSet(set.id, set.name)}
                className="p-1.5 text-gray-400 hover:text-blue-500 rounded-full opacity-0 group-hover:opacity-100 hover:bg-gray-100 flex-shrink-0"
              >
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
